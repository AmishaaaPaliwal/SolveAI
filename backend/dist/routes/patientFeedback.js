...
const patientFeedback_1 = require("../services/patientFeedback");
...
router.get('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        // Check cache first
        let feedback = await redis_1.redisService.getCachedPatientData(id);
        if (!feedback) {
            // Fetch from Firestore
            feedback = await patientFeedback_1.patientFeedbackService.getById(id);
            if (feedback) {
                // Cache for 1 hour
                await redis_1.redisService.cachePatientData(id, feedback);
            }
        }
        if (!feedback) {
...
router.post('/', async (req, res) => {
    try {
        ...
        const newFeedback = await patientFeedback_1.patientFeedbackService.create(feedbackData);
        // Invalidate cache
        await redis_1.redisService.delete('patientFeedback:all');
        res.status(201).json({
            success: true,
            data: newFeedback,
...
router.put('/:id', async (req, res) => {
    try {
        ...
        await patientFeedback_1.patientFeedbackService.update(id, updateData);
        // Invalidate cache
        await redis_1.redisService.delete(`patientFeedback:${id}`);
        await redis_1.redisService.delete('patientFeedback:all');
        res.json({
            success: true,
...
router.delete('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        await patientFeedback_1.patientFeedbackService.delete(id);
        // Invalidate cache
        await redis_1.redisService.delete(`patientFeedback:${id}`);
        await redis_1.redisService.delete('patientFeedback:all');
        res.json({
